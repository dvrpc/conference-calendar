...
export default function EditEvent({
  ...
  loaderData: { calendarId: string; event: CalendarEvent };
}) {
  const { calendarId, event } = loaderData;
  ...
  const [initialEndDate] = useState(endDateTime.split("T")[0] || "");
  ...
  const existingCommittee = event.extendedProperties?.private?.committee || "";
  const [selectedCommittee, setSelectedCommittee] = useState(existingCommittee);
  const startTime = startDateTime.split("T")[1]?.slice(0, 5) || "";
  const endTime = endDateTime.split("T")[1]?.slice(0, 5) || "";

  const existingTags: string[] = [
    event.extendedProperties?.private?.tag1,
    event.extendedProperties?.private?.tag2,
    event.extendedProperties?.private?.tag3,
    event.extendedProperties?.private?.tag4,
  ...
  const handleStartDateChange = (newStartDate: string) => {
    ...
    if (!initialEndDate || endDate === initialEndDate || !endDate) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <title>Edit Event - Conference Calendar Admin</title>
      <div className="mb-6">
        <Link
          to={`/?calendarId=${encodeURIComponent(calendarId)}`}
        ...
        <form method="post">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <UrlField name="description" defaultValue={event.description || ""} />
            </div>
            ...
            <div>
              ...
              <TagsInput name="tags" defaultValue={existingTags} />
            </div>
          </div>
